...
import {
  LaunchCard,
  type SpaceXLaunch,
} from "@/components/molecules/LaunchCard.tsx";
import { useAuth } from "@/contexts/useAuth.ts";
import { Pagination } from "@/components/molecules/Pagination.tsx";
import { SpaceXService } from "@/services/api.ts";
...
export default function ExplorePage() {
  ...
  const fetchLaunches = useCallback(
    async (pageNumber: number) => {
      ...
      try {
        const data: SpaceXApiResponse = await SpaceXService.getLaunches(
          token,
          pageNumber,
          10
        );
    ...
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <main
        className={`
          ...
          `}
      >
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 md:ml-0 ml-16">
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white flex items-center gap-3">
            Missões SpaceX
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Explore o histórico de lançamentos consumindo dados da API oficial
            via Backend.
          </p>
        </div>
        ...
        {error && (
          <div className="p-6 bg-red-50 border border-red-200 rounded-xl text-center text-red-600 mb-6">
            <p>{error}</p>
            <Button
              onClick={() => fetchLaunches(pagination.page)}
              variant="outline"
